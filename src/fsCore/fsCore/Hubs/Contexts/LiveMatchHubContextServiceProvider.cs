...
namespace fsCore.Hubs.Contexts
{
    public class LiveMatchHubContextServiceProvider : ILiveMatchHubContextServiceProvider
    {
        ...
        {
            await HubContextServiceProviderMethodExceptionWrapper((() => UpdateMatchForClientsLogic(matchId), nameof(UpdateMatchForClients)));
        }
        ...
        {
            _logger.LogInformation("LiveMatch signal R method: {MethodName} started executing", hubAction.HubMethodName);
            try
            {
            ...
            {
                _logger.LogError("Signal R method: {MethodName} failed with exception message {Exception} and status code {StatusCode}", hubAction.HubMethodName, e.Message, e.StatusCode);
            }
            catch (Exception e)
            {
                _logger.LogError("Signal R method: {MethodName} failed with exception message {Exception}", hubAction.HubMethodName, e.Message);
            }
            finally
            {
                _logger.LogInformation("LiveMatch signal R method: {MethodName} finished executing", hubAction.HubMethodName);
            }
        }
    }
}