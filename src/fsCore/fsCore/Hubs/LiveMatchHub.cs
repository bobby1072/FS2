...
namespace fsCore.Hubs
{
    ...
    public class LiveMatchHub : BaseHub
    {
        ...
        private async Task SaveCatchLogic(SaveMatchCatchLiveMatchHubInput hubInput)
        {
            ...
            await _liveMatchService.SaveCatch(
                hubInput.MatchId,
                hubInput.ToLiveMatchCatch((Guid)user.Id!),
                user
            );

            var match =
                await _liveMatchPersistenceService.TryGetLiveMatch(hubInput.MatchId)
                ?? throw new ApiException(
                    LiveMatchConstants.LiveMatchDoesntExist,
            ...
            await Clients
                .Group($"{LiveMatchGroupPrefix}{hubInput.MatchId}")
                .SendAsync(UpdateMatchMessage, HubResponseBuilder.FromLiveMatch(match));
        }
        ...
        private async Task EndMatchLogic(Guid matchId)
        {
            ...
            await Clients
                .Group($"{LiveMatchGroupPrefix}{matchId}")
                .SendAsync(UpdateMatchMessage, HubResponseBuilder.FromLiveMatch(match));
        }
        ...
        private async Task UpdateMatchLogic(SaveMatchLiveHubInput matchInput)
        {
            ...
            var match =
                await _liveMatchPersistenceService.TryGetLiveMatch(liveMatch.Id)
                ?? throw new ApiException(
                    LiveMatchConstants.LiveMatchDoesntExist,
                    HttpStatusCode.NotFound
                );
            ...
            await Clients
                .Group($"{LiveMatchGroupPrefix}{match.Id}")
                .SendAsync(UpdateMatchMessage, HubResponseBuilder.FromLiveMatch(match));
        }

        public async Task SaveParticipant(SaveParticipantLiveMatchHubInput hubInput)
        {
            Task saveParticipantLocalMethod() =>
                SaveParticipantLogic(hubInput.UserId, hubInput.MatchId);
        ...
        private async Task SaveParticipantLogic(Guid userId, Guid matchId)
        {
            ...
            var foundUserConnection = await _cachingService.TryGetObject<string>(
                $"{RequiredSignalRUserConnectionId.ConnectionIdUserIdCacheKeyPrefix}{userId}"
            );
        ...
        private async Task CreateMatchLogic(SaveMatchLiveHubInput liveMatchInput)
        {
            ...
            await Groups.AddToGroupAsync(
                ...
                $"{LiveMatchGroupPrefix}{createdMatch.Id}"
            );

            createdMatch.RemoveSensitive();

            await Clients
                .Group($"{LiveMatchGroupPrefix}{createdMatch.Id}")
                .SendAsync(CreateMatchMessage, HubResponseBuilder.FromLiveMatch(createdMatch));
        }
        ...
        private async Task OnConnectedAsyncLogic()
        {
            ...
            await Task.WhenAll(
                AddUsersToMatchGroup(allMatchesForUser, Context.ConnectionId),
                allMatchesJob
            );

            var allMatches = await allMatchesJob;
            var toUpdateParticipants =
        ...
        {
            await HubMethodExceptionWrapper(
                (() => OnDisconnectedAsyncLogic(exception), nameof(OnDisconnectedAsync))
            );
        }
        ...
        private async Task OnDisconnectedAsyncLogic(Exception? exception)
        {
            if (exception is not null)
            {
                _logger.LogError(
                    exception,
                    "Signal R connection {ConnectionId} disconnected with exception message {Exception}",
                    Context.ConnectionId,
                    exception.Message
                );
            }
            ...
            await Task.WhenAll(
                _cachingService.TryRemoveObject(
                    $"{RequiredSignalRUserConnectionId.ConnectionIdUserIdCacheKeyPrefix}{user.Id}"
                ),
        ...
        private async Task HubMethodExceptionWrapper(
        ...
        )
        {
            try
            {
                _logger.LogInformation(
                    "LiveMatch signal R method: {MethodName} started executing for connection {ConnectionId}",
            ...
            catch (ApiException e)
            {
                ...
                _logger.LogError(
                    e,
                    "Signal R method: {MethodName} for connection {ConnectionId} failed with exception message {Exception} and status code {StatusCode}",
                    hubAction.HubMethodName,
                    Context.ConnectionId,
                    e.Message,
                    e.StatusCode
            ...
            catch (ValidationException e)
            {
                ...
                _logger.LogError(
                    e,
                    "Signal R method: {MethodName} for connection {ConnectionId} failed with exception message {Exception}",
                    hubAction.HubMethodName,
                    Context.ConnectionId,
                    e.Message
                );
            ...
            {
                _logger.LogInformation(
                    "LiveMatch signal R method: {MethodName} finished executing for connection {ConnectionId}",
                    hubAction.HubMethodName,
                    Context.ConnectionId
                );
            }
        ...
        private async Task AddUsersToMatchGroup(ICollection<Guid> matchIds, string connectionId)
        {
            ...
            foreach (var matchId in matchIds)
            {
                jobList.Add(
                    Groups.AddToGroupAsync(
                        connectionId,
                        $"{LiveMatchGroupPrefix}{matchId.ToString()}"
                    )
                );
            }
            await Task.WhenAll(jobList);
