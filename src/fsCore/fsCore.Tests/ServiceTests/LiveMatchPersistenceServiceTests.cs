...
namespace fsCore.Tests.ServiceTests
{
    public class LiveMatchPersistenceServiceTests : LiveMatchTestBase
    {
        ...
        public LiveMatchPersistenceServiceTests()
        {
            ...
            _mockActiveLiveMatchParticipantRepository =
                new Mock<IActiveLiveMatchParticipantRepository>();
            _liveMatchPersistenceService = new LiveMatchPersistenceService(
                _mockCachingService.Object,
                _mockActiveLiveMatchCatchRepository.Object,
                _mockActiveLiveMatchRepository.Object,
                _mockActiveLiveMatchParticipantRepository.Object
            );
        }
        ...
        public async Task TryGetMatch_Should_Return_Match_If_Cached()
        {
            ...
            _mockCachingService
                .Setup(x =>
                    x.TryGetObject<LiveMatchJsonType>($"{_liveMatchKey}{liveMatch.Id.ToString()}")
                )
                .ReturnsAsync(liveMatch.ToJsonType());
            ...
            _mockCachingService.Verify(
                x => x.TryGetObject<LiveMatchJsonType>($"{_liveMatchKey}{liveMatch.Id.ToString()}"),
                Times.Once
            );
            _mockActiveLiveMatchRepository.Verify(
        ...
        public async Task TryGetMatch_Should_Return_MatchFrom_Db_If_Cached_Returns_Null()
        {
            ...
            var liveMatch = CreateLiveMatch();
            _mockCachingService
                .Setup(x => x.TryGetObject<LiveMatchJsonType>(It.IsAny<string>()))
                .ReturnsAsync((LiveMatchJsonType?)null);
            _mockActiveLiveMatchRepository
                .Setup(x => x.GetFullOneById(liveMatch.Id))
        ...
        public async Task SetLiveMatch_Should_Create_The_Match_If_Does_Not_Exist(
        ...
        )
        {
            ...
            _mockActiveLiveMatchRepository
                .Setup(x => x.GetFullOneById(It.IsAny<Guid>()))
                .ReturnsAsync((LiveMatch?)null);
            _mockCachingService
                .Setup(x =>
                    x.TryGetObject<LiveMatchJsonType>($"{_liveMatchKey}{liveMatch.Id.ToString()}")
                )
            ...
            _mockActiveLiveMatchRepository
                .Setup(x =>
                    x.Create(
                        It.Is<ICollection<LiveMatch>>(y => y.FirstOrDefault()!.Id == liveMatch.Id)
                    )
                )
                .ReturnsAsync([liveMatch]);
            _mockCachingService
                .Setup(x =>
                    x.SetObject(
                        $"{_liveMatchKey}{liveMatch.Id.ToString()}",
                        It.Is<LiveMatchJsonType>(y => y.Id == liveMatch.Id),
                        It.IsAny<DistributedCacheEntryOptions>()
                    )
                )
                .ReturnsAsync("fdgfdsgyfdsghfdsghghsdj");
        ...
        [InlineData(LiveMatchStatus.NotStarted)]
        [InlineData(LiveMatchStatus.InProgress)]
        public async Task SetLiveMatch_Should_Update_And_Set_Object_If_Exists(
            LiveMatchStatus status
        )
        {
            //Arrange
            ...
            _mockCachingService
                ...
                .ReturnsAsync(oldLiveMatch.ToJsonType());
            _mockActiveLiveMatchRepository
                .Setup(x =>
                    x.Update(
                        It.Is<ICollection<LiveMatch>>(y => y.FirstOrDefault()!.Id == liveMatch.Id)
