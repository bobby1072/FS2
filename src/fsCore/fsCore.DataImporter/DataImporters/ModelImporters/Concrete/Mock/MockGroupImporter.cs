...
namespace fsCore.DataImporter.DataImporters.ModelImporters.Concrete.Mock
{
    internal class MockGroupImporter : IGroupImporter
    {
        ...
        public MockGroupImporter(
            IGroupRepository groupRepository,
            IUserRepository userRepo,
            ILogger<MockGroupImporter> logger
        )
        {
            _logger = logger;
            ...
            while (tryCount < 3)
            {
                try
                {
                    var allUsers =
                        ...
                        ?? throw new InvalidOperationException("Cannot get all users");
                    var listOfGroupsToCreate = new List<Group>();
                    for (int x = 0; x < allUsers.Count; x++)
                    {
                        var currentGroupList = new Group[
                            (int)NumberOfMockModelToCreate.GroupsPerUser
                        ];
                        ...
                        for (int deepX = 0; deepX < currentGroupList.Length; deepX++)
                        {
                            ...
                            currentGroupList[deepX] = CreateUniqueGroupNameGroup(
                                tempCheckList,
                                foundUser?.Id
                    ...
                    var createdGroups =
                        await _groupRepository.Create(listOfGroupsToCreate)
