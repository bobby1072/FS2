...
namespace fsCore.Services.Concrete
{
    public class LiveMatchService : ILiveMatchService
    {
        ...
        public async Task<ICollection<LiveMatch>> AllMatchesParticipatedIn(
            ICollection<Guid> matchIds
        )
        {
        ...
        public async Task<ICollection<Guid>> AllMatchesParticipatedIn(
        ...
        )
        {
            ...
            {
                throw new LiveMatchException(
                    LiveMatchConstants.LiveMatchHasEnded,
                    HttpStatusCode.BadRequest
                );
            }
            if (
                !currentUser.GroupPermissions.Can(PermissionConstants.Manage, foundMatch.GroupId)
                || currentUser.Id != foundMatch.MatchLeaderId
            )
            {
                throw new LiveMatchException(
            ...
            {
                throw new LiveMatchException(
                    LiveMatchConstants.LiveMatchIsFull,
                    HttpStatusCode.BadRequest
                );
            }
        ...
        public async Task<LiveMatch> CreateMatch(
            LiveMatch match,
            UserWithGroupPermissionSet currentUser
        )
        {
            ...
            {
                throw new LiveMatchException(
                    LiveMatchConstants.LiveMatchAlreadyExists,
            ...
            if (match.CommencesAt is not null)
            {
                _backgroundJobClient.Schedule(
                    HangfireConstants.Queues.LiveMatchJobs,
                    () => AutomaticStartMatch(match.Id, currentUser),
                    match.TimeUntilStart!.Value
                );
            }
            if (match.EndsAt is not null)
            {
                _backgroundJobClient.Schedule(
                    HangfireConstants.Queues.LiveMatchJobs,
                    () => AutomaticEndMatch(match.Id, currentUser),
                    match.TimeUntilEnd!.Value
                );
            }
        ...
        {
            var foundMatch =
                await _liveMatchPersistenceService.TryGetLiveMatch(match.Id)
                ?? throw new LiveMatchException(
                    LiveMatchConstants.LiveMatchHasMissingOrIncorrectDetails,
                    HttpStatusCode.BadRequest
                );
        ...
        public async Task SaveCatch(
            Guid matchId,
            LiveMatchCatch liveMatchCatch,
            UserWithGroupPermissionSet currentUser
        )
        {
            var foundMatch =
            ...
            {
                throw new LiveMatchException(
                    LiveMatchConstants.LiveMatchHasntStarted,
                    HttpStatusCode.BadRequest
                );
            }
            ...
            if (
                currentUser.Id != liveMatchCatch.UserId
                && currentUser.Id != foundMatch.MatchLeaderId
            )
            {
                throw new LiveMatchException(
            ...
            await Task.WhenAll(
                _liveMatchCatchValidator.ValidateAndThrowAsync(liveMatchCatch),
                isCatchValidJob
            );

            var isCatchValid = (await isCatchValidJob).IsValid;
            if (
                foundMatch.Catches.FirstOrDefault(x => x.Id == liveMatchCatch.Id)
        ...
        public async Task<LiveMatch> StartMatch(
            Guid matchId,
        ...
        )
        {
            ...
            if (
                ...
                || userWithGroupPermissionSet.Id != foundMatch.MatchLeaderId
            )
            {
                throw new LiveMatchException(
                    ErrorConstants.DontHavePermission,
        ...
        public async Task<LiveMatch> EndMatch(
            Guid matchId,
            UserWithGroupPermissionSet userWithGroupPermissionSet
        )
        {
        ...
        public async Task AutomaticStartMatch(
            Guid matchId,
            UserWithGroupPermissionSet userWithGroupPermissionSet
        )
        {
        ...
        public async Task AutomaticEndMatch(
            Guid matchId,
            UserWithGroupPermissionSet userWithGroupPermissionSet
        )
        {
