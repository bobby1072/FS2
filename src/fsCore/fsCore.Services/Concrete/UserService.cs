...
namespace fsCore.Services.Concrete
{
    public class UserService : IUserService
    {
        ...
        {
            var foundUser = await _repo.GetOne(
                id,
                typeof(User).GetProperty("Id".ToPascalCase())?.Name ?? throw new Exception()
            );
            return foundUser
                ?? throw new ApiException(ErrorConstants.NoUserFound, HttpStatusCode.NotFound);
        }
        ...
        public async Task<User> GetUser(string email)
        {
            ...
            var foundDetail =
                userProperties.FirstOrDefault(x =>
                {
        ...
        public async Task<User> SaveUser(User user)
        {
            ...
            if (foundUser != null)
            {
                if (!user.ValidateAgainstOriginal(foundUser))
                    throw new ApiException(
                        ErrorConstants.NotAllowedToEditThoseFields,
                        HttpStatusCode.BadRequest
                    );
                return (await _repo.Update(new List<User> { user }))?.FirstOrDefault()
                    ?? throw new ApiException(
                        ErrorConstants.CantCreateUser,
                        HttpStatusCode.InternalServerError
                    );
            }
            return (await _repo.Create(new List<User> { user }))?.FirstOrDefault()
                ?? throw new ApiException(
                    ErrorConstants.CantCreateUser,
                    HttpStatusCode.InternalServerError
                );
