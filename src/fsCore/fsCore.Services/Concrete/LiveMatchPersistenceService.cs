...
namespace fsCore.Services.Concrete
{
    public class LiveMatchPersistenceService : ILiveMatchPersistenceService
    {
        ...
        public LiveMatchPersistenceService(
            ICachingService cachingService,
            IActiveLiveMatchCatchRepository activeLiveMatchCatchRepository,
            IActiveLiveMatchRepository activeLiveMatchRepository,
            IActiveLiveMatchParticipantRepository activeLiveMatchParticipantRepository
        )
        {
        ...
        public async Task SaveParticipant(
            ICollection<(LiveMatchParticipant LiveMatchParticipant, Guid MatchId)> participants
        )
        {
            var matches =
                await _activeLiveMatchRepository.GetFullOneById(
                    participants.Select(x => x.MatchId).ToArray()
                )
                ?? throw new LiveMatchException(
                    LiveMatchConstants.LiveMatchHasMissingOrIncorrectDetails,
                    HttpStatusCode.BadRequest
                );
            var userToUpdateList =
                new List<(LiveMatchParticipant LiveMatchParticipant, Guid MatchId)>();
            var userToCreateList =
                new List<(LiveMatchParticipant LiveMatchParticipant, Guid MatchId)>();
            var cacheJobList = new List<Task<string>>();
            foreach (var participant in participants)
            {
                var foundLiveMatch =
                    matches.FirstOrDefault(m => m.Id == participant.MatchId)
                    ?? throw new LiveMatchException(
                        LiveMatchConstants.LiveMatchHasMissingOrIncorrectDetails,
                        HttpStatusCode.BadRequest
                    );
                if (
                    foundLiveMatch.Participants.FirstOrDefault(p =>
                        p.Id == participant.LiveMatchParticipant.Id
                    )
                    is null
                )
                {
                    ...
                    if (
                        foundLiveMatch.MatchStatus == LiveMatchStatus.InProgress
                        || foundLiveMatch.MatchStatus == LiveMatchStatus.Finished
                    )
                    {
                        foundLiveMatch.Participants = foundLiveMatch
                            .Participants.Append(participant.LiveMatchParticipant)
                            .ToList();
                        cacheJobList.Add(
                            _cachingService.SetObject(
                                $"{_liveMatchKey}{foundLiveMatch.Id}",
                                foundLiveMatch.ToJsonType(),
                                GetTimeToCache(foundLiveMatch)
                            )
                        );
                    }
                }
            ...
            {
                userToUpdateList.Count > 0
                    ? _activeLiveMatchParticipantRepository.Update(userToUpdateList)
                    : Task.CompletedTask,
                userToCreateList.Count > 0
                    ? _activeLiveMatchParticipantRepository.Create(userToCreateList)
                    : Task.CompletedTask,
        ...
        {
            var foundLiveMatch =
                await TryGetLiveMatch(liveMatchId)
                ?? throw new LiveMatchException(
                    LiveMatchConstants.LiveMatchHasMissingOrIncorrectDetails,
                    HttpStatusCode.BadRequest
                );
            if (foundLiveMatch.Participants.FirstOrDefault(p => p.Id == user.Id) is User foundUser)
            {
                ...
                {
                    foundLiveMatch.Participants = foundLiveMatch
                        .Participants.Where(p => p.Id != user.Id)
                        .ToList();
        ...
        public async Task DeleteParticipant(
            Guid liveMatchId,
            ICollection<LiveMatchParticipant> user
        )
        {
            var foundLiveMatch =
                await TryGetLiveMatch(liveMatchId)
            ...
            if (
                foundLiveMatch.Participants.FirstOrDefault(p =>
                    user.FirstOrDefault(x => p.Id == x.Id) is not null
                )
                is not null
            )
            {
                ...
                {
                    var participants = await _activeLiveMatchParticipantRepository.GetForMatch(
                        liveMatchId
                    );
        ...
        public async Task SaveCatch(Guid liveMatchId, LiveMatchCatch liveMatchCatch)
        {
            ...
            if (
                foundLiveMatch.Catches.FirstOrDefault(c => c.Id == liveMatchCatch.Id)
                is LiveMatchCatch foundCatch
            )
            {
                ...
                {
                    foundLiveMatch.Catches = foundLiveMatch
                        .Catches.Select(c => c.Id == liveMatchCatch.Id ? liveMatchCatch : c)
                        .ToList();
                    await _cachingService.SetObject(
                        $"{_liveMatchKey}{foundLiveMatch.Id}",
                        foundLiveMatch.ToJsonType(),
        ...
        public async Task DeleteCatch(Guid liveMatchId, LiveMatchCatch liveMatchCatch)
        {
            ...
            )
            {
                ...
                {
                    foundLiveMatch.Catches = foundLiveMatch
                        .Catches.Where(c => c.Id != liveMatchCatch.Id)
                        .ToList();
                    await _cachingService.SetObject(
                        $"{_liveMatchKey}{foundLiveMatch.Id}",
                        foundLiveMatch.ToJsonType(),
        ...
        public async Task SetLiveMatch(LiveMatch liveMatch)
        {
            ...
            else if (foundLiveMatch is null)
            {
                ...
                {
                    if (
                        liveMatch.MatchStatus == LiveMatchStatus.InProgress
                        || liveMatch.MatchStatus == LiveMatchStatus.Finished
                    )
                    {
                        await _cachingService.SetObject(
                            $"{_liveMatchKey}{liveMatch.Id}",
                            liveMatch.ToJsonType(),
                            GetTimeToCache(liveMatch)
                        );
                    }
                }
                else
                {
                    throw new LiveMatchException(
                        LiveMatchConstants.FailedToPersistLiveMatch,
        ...
        public async Task<LiveMatch?> TryGetLiveMatch(Guid matchId)
        {
            try
            {
                var cacheType = await _cachingService.TryGetObject<LiveMatchJsonType>(
                    $"{_liveMatchKey}{matchId}"
                );
        ...
        private static DistributedCacheEntryOptions GetTimeToCache(LiveMatch liveMatch)
        {
            ...
            return new DistributedCacheEntryOptions
            {
                AbsoluteExpirationRelativeToNow = liveMatch.TimeUntilEnd,
            };
        }
    }
}
