...
export const PartialGroupCatchLeaderBoard: React.FC<{
  partialCatches?: RuntimePartialGroupCatchModel[];
}> = ({ partialCatches = [] }) => {
  const [leaderBoardType, setLeaderBoardType] =
    usePersistedState<LeaderBoardType>(
      "catchLeaderBoardType",
      LeaderBoardType.MostRecent
    );
    ...
    },
    {
      ...
      options: {
        ...
        customBodyRender: (val: Date) => (
          <Typography>{prettyDateWithTime(val)}</Typography>
        ),
        sort: false,
      },
    },
  ...
  const options: MUIDataTableOptions = {
    ...
    customToolbar: () => (
      <div style={{ padding: 0.1 }}>
        <FormControl>
          <InputLabel>Leader board type</InputLabel>
          <Select
            fullWidth
  ...
  return (
    <MUIDataTable
      ...
      columns={columns}
      title={"Group catch leader board"}
      options={options}
    />
  );
};
